...
<script>
import { getCommentList, postCommentList } from '@/apis/article.js'
...
export default {
  ...
  methods: {
    ...
    async sendComment () {
      ...
      let res = await postCommentList(this.post.id, data)
      console.log(res)
    ...
    async starThisArticle () {
      let res = await getCommentList(this.post.id)
