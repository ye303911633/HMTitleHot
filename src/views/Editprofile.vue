<template>
  ...
  <!--    内容cell单元格-->
  <van-cell title="昵称" is-link :value="user.nickname" @click="show=!show"/>
  <van-dialog
    v-model="show"
    title="昵称"
    show-cancel-button
  ...
  <van-cell title="密码" is-link :value="user.password" @click="show1=!show1"/>
  <van-dialog
    v-model="show1"
    title="密码"
    show-cancel-button
  ...
  >
    ...
    <van-cell title="性别" is-link :value="user.gender" @click="show2=!show2"/>
    <van-dialog
      v-model="show2"
      title="性别"
      show-cancel-button
...
export default {
  ...
  methods: {
    ...
    async getGender () {
      let num = this.$refs.nick.$refs.input.value
      let res = await uploadFile(this.user.id, {password: num})
      if (res.data.message === '修改成功') {
        this.$toast.success(res.data.message)
